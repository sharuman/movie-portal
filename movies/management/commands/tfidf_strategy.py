...
def getTfIdfRecommendations(movie_id: int, cut_off: int = 5) -> pd.DataFrame:
    ...
    """
    movies_df = pd.DataFrame.from_records(Movie.objects.all().values('id', 'title', 'plot', 'poster_path'))
    ...
    recommendations = movies_df.sort_values(by=['score'], ascending=False)[:cut_off + 1]
